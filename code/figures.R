...
#   for preparation of the data: data_assembly_20210217.R (and subsequent)
#   for data exploration and model building: model_comparison_20210227.R (and subsequent)
...
names(site_means_renamed)[names(site_means_renamed)=="cloudmean"] <- "Cloud cover"
...
site_means_renamed$Eelgrass.biomass <- -site_means_renamed$PC2.zos.site
names(site_means_renamed)[names(site_means_renamed)=="Eelgrass.biomass"] <- "Eelgrass biomass (-PCz2)"
...
names(site_means_renamed)[names(site_means_renamed)=="log10.Zostera.shoots.core.site"] <- "Shoot density (log)"
...
site_means_renamed_Atlantic <- droplevels(subset(site_means_renamed, Ocean == "Atlantic"))
site_means_renamed_Pacific <- droplevels(subset(site_means_renamed, Ocean == "Pacific"))
...
pairs.panels(site_means_renamed[,c("Latitude", "Env PCe1", "Env PCe2", 
...
pairs.panels(site_means_renamed[,c("Eelgrass form (PCz1)", "Eelgrass biomass (-PCz2)",
...
# pairs.panels(site_means_renamed_Atlantic[,c("Latitude", "Env PCe1", "Env PCe2", "Env PCe3", 
...
# pairs.panels(site_means_renamed_Pacific[,c("Latitude", "Env PCe1", "Env PCe2", "Env PCe3", 
#   "Genetics FCA1", "Genetics FCA2", "Eelgrass form (PCz1)", "Eelgrass biomass (-PCz2)",
#   "Periphyton mass (log)", "Mesograzer mass (log)")], hist.col="gray", pch = 21, 
...
pc1z.pc2z <- ggplot(site_means, aes(x = PC1.zos.site, y = PC2.zos.site, group = Ocean, col = Ocean)) +
  geom_point(size = 4) +
...
fc1.fc2 <- ggplot(site_means, aes(x = FC1, y = FC2, group = Ocean, col = Ocean)) +
  geom_point(size = 4) +
  geom_text(aes(label = unique(Site)), hjust = -0.25, vjust = 0, size = 3) +
  scale_color_manual(values = c("blue", "forestgreen")) +
...
library(tidyverse)
hull_PC12 <- site_means %>%
  group_by(Ocean) %>%
  slice(chull(PC1.env.global, PC2.env.global))
pce1.pce2 <- ggplot(site_means, aes(x = PC1.env.global, y = PC2.env.global, group = Ocean, col = Ocean, fill = Ocean)) +
...
hull_PC23 <- site_means %>%
  group_by(Ocean) %>%
  slice(chull(PC2.env.global, PC3.env.global))
pce2.pce3 <- ggplot(site_means, aes(x = PC2.env.global, y = PC3.env.global, group = Ocean, col = Ocean, fill = Ocean)) +
...
# pc1z.fc1 = ggplot(site_means, aes(x = FC1, y = PC1.zos.site, group = Ocean, col = Ocean)) +
#   geom_point(size = 4) +
#   # geom_text(aes(label = unique(Site)), hjust = -0.25, vjust = 0, size = 3) +
#   scale_color_manual(values = c("blue", "forestgreen")) +
#   xlab("Eelgrass genetics (FCA1)") +  
...
pc1z.fc2 = ggplot(site_means, aes(x = FC2, y = PC1.zos.site, group = Ocean, col = Ocean)) +
  geom_point(size = 4) +
  # geom_text(aes(label = unique(Site)), hjust = -0.25, vjust = 0, size = 3) +
  scale_color_manual(values = c("blue", "forestgreen")) +
...
shoots.canopy <- ggplot(site_means, aes(x = Zostera.shoots.core.site, y = Zostera.longest.leaf.length.site, group = Ocean, col = Ocean)) +
  geom_point(size = 4) +
  geom_text(aes(label = unique(Site)), hjust = -0.25, vjust = 0, size = 3) +
  scale_color_manual(values = c("blue", "forestgreen")) +
  xlab("Shoot density") +  
...
shoots.zag <- ggplot(site_means, aes(x = log10.Zostera.shoots.core.site, y = log10.Zostera.AG.mass.site, group = Ocean, col = Ocean)) +
  geom_point(size = 4) +
  geom_text(aes(label = unique(Site)), hjust = -0.25, vjust = 0, size = 3) +
  scale_color_manual(values = c("blue", "forestgreen")) +
  xlab("Shoot density") +  
...
zen_best_models_atlantic <- read.csv("data/evol_envir_path_calculations/Duffy_et_al_2022_model_coeffs_atlantic.csv",header = TRUE)
...
zen_best_models_pacific <- read.csv("data/evol_envir_path_calculations/Duffy_et_al_2022_model_coeffs_pacific.csv",header = TRUE)
...
site_means_49_Atlantic$rPCe3.rFC1.atl <- site_means_49_Atlantic$rPC3.env.global.atl * site_means_49_Atlantic$rFC1.global.atl
...
pcz1.pce1.a.partial <- partialResid(rPC1.zos.atl ~ rPC1.env.global.atl, pcz1.site.a.4.x, site_means_49_Atlantic) 
# Add Ocean and site to residuals data frame 
Ocean <- site_means_49_Atlantic$Ocean
Site <- site_means_49_Atlantic$Site
...
# Extract residuals from model
pcz1.fc2.a.partial <- partialResid(rPC1.zos.atl ~ rFC2.global.atl, pcz1.site.a.4.x, site_means_49_Atlantic) 
# Add Ocean and site to residuals data frame 
Ocean <- site_means_49_Atlantic$Ocean
Site <- site_means_49_Atlantic$Site
...
site_means_49_Atlantic$rPCe1.rFC2.atl <- site_means_49_Atlantic$rPC1.env.global.atl * site_means_49_Atlantic$rFC2.global.atl
pcz2.test.a.2.x <- lm(rPC2.zos.atl ~ 
                    ...
                    , data = site_means_49_Atlantic)
# Extract residuals from model
pcz2.pce1.a.partial <- partialResid(rPC2.zos.atl ~ rPC1.env.global.atl, pcz2.test.a.2.x, site_means_49_Atlantic) 
# Add Ocean and site to residuals data frame 
Ocean <- site_means_49_Atlantic$Ocean
Site <- site_means_49_Atlantic$Site
...
# Extract residuals from model
pcz2.pce3.a.partial <- partialResid(rPC2.zos.atl ~ rPC3.env.global.atl, pcz2.test.a.2.x, site_means_49_Atlantic) 
...
# Extract residuals from model
pcz2.fc2.a.partial <- partialResid(rPC2.zos.atl ~ rFC2.global.atl, pcz2.test.a.2.x, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
site_means_49_Atlantic$rPCz1.rPCe2.atl <- site_means_49_Atlantic$rPC1.zos.atl*site_means_49_Atlantic$rPC2.env.global.atl
peri.test.a.9.x <- lm(rperiphyton.perg.atl ~ 
                        + rPC1.env.global.atl + rPC2.env.global.atl + rPC3.env.global.atl + rFC1.global.atl + rFC2.global.atl
                      + rPC1.zos.atl + rPC2.zos.atl + rPCz1.rPCe2.atl # added
                      , data = site_means_49_Atlantic)
peri.pcz1.a.partial <- partialResid(rperiphyton.perg.atl ~ rPC1.zos.atl, peri.test.a.9.x, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
peri.area.test.a.9 <- lm(rperiphyton.area.atl ~ 
                         ...
                         , data = site_means_49_Atlantic)
peri.pcz1.a.partial <- partialResid(rperiphyton.area.atl ~ rPC1.zos.atl, peri.area.test.a.9, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
peri.pcz1.a.partial <- cbind(peri.pcz1.a.partial, Site)
peri.pcz1.a.partial.plot <- ggplot(peri.pcz1.a.partial, aes(x = xresid, y = yresid, col = "blue")) +
...
peri.pcz2.a.partial <- partialResid(rperiphyton.area.atl ~ rPC2.zos.atl, peri.area.test.a.9, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
peri.area.test.a.9 <- lm(rperiphyton.area.atl ~ 
                         ...
                         + rPCz1.rPCe2.atl # added
                         , data = site_means_49_Atlantic)
peri.pce2.a.partial <- partialResid(rperiphyton.area.atl ~ rPC2.env.global.atl, peri.area.test.a.9, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
peri.test.a.9.x <- lm(rperiphyton.perg.atl ~ 
                      ...
                      + rPC1.zos.atl + rPC2.zos.atl + rPCz1.rPCe2.atl # added
                      , data = site_means_49_Atlantic)
peri.pcz2.a.partial <- partialResid(rperiphyton.perg.atl ~ rPC2.zos.atl, peri.test.a.9.x, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
peri.pcz2.a.partial <- cbind(peri.pcz2.a.partial, Site)
peri.pcz2.a.partial.plot <- ggplot(peri.pcz2.a.partial, aes(x = xresid, y = -yresid, col = "blue")) +
...
meso.test.a.9.x <- lm(rmesograzer.mass.perg.atl ~ 
  ...
  , data = site_means_49_Atlantic)
# Extract residuals from model
meso.pce3.a.partial <- partialResid(rmesograzer.mass.perg.atl ~ rPC3.env.global.atl, meso.test.a.9.x, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
# Extract residuals from model
meso.fc2.a.partial <- partialResid(rmesograzer.mass.perg.atl ~ rFC2.global.atl, meso.test.a.9.x, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
# # Extract residuals from model
# meso.pcz1.a.partial <- partialResid(rmesograzer.mass.perg.atl ~ rPC1.zos.atl, meso.test.a.9.x, site_means_49_Atlantic) 
# # Add site to residuals data frame 
# Site <- site_means_49_Atlantic$Site
...
# # Extract residuals from model
# meso.perg.pcz2.a.partial <- partialResid(rmesograzer.mass.perg.atl ~ rPC2.zos.atl, meso.test.a.9.x, site_means_49_Atlantic) 
# # Add site to residuals data frame 
# Site <- site_means_49_Atlantic$Site
...
meso.area.test.a.1 <- lm(rmesograzer.mass.area.atl ~ 
                         ...
                         , data = site_means_49_Atlantic)
# Extract residuals from model
meso.pcz1.a.partial <- partialResid(rmesograzer.mass.area.atl ~ rPC1.zos.atl, meso.area.test.a.1, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
meso.pcz1.a.partial <- cbind(meso.pcz1.a.partial, Site)
meso.pcz1.a.partial.plot <- ggplot(meso.pcz1.a.partial, aes(x = xresid, y = yresid, col = "blue")) +
...
# Extract residuals from model
meso.area.pcz2.a.partial <- partialResid(rmesograzer.mass.area.atl ~ rPC2.zos.atl, meso.area.test.a.1, site_means_49_Atlantic) 
# Add site to residuals data frame 
Site <- site_means_49_Atlantic$Site
...
# # Extract residuals from model
# meso.area.pce3.a.partial <- partialResid(rmesograzer.mass.area.atl ~ rPC3.env.global.atl, meso.area.test.a.1, site_means_49_Atlantic) 
# # Add site to residuals data frame 
# Site <- site_means_49_Atlantic$Site
...
# # Extract residuals from model
# pcz1.pce1.p.partial <- partialResid(rPC1.zos.pac ~ rPC1.env.global.pac, pcz1.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# Extract residuals from model
pcz1.fc2.p.partial <- partialResid(rPC1.zos.pac ~ rFC2.global.pac, pcz1.test.p.1, site_means_Pacific) 
# Add site to residuals data frame 
Site <- site_means_Pacific$Site
...
#                     , data = site_means_Pacific)
# # Extract residuals from model
# pcz2.pce1.p.partial <- partialResid(rPC2.zos.pac ~ rPC1.env.global.pac, pcz2.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
#                     , data = site_means_Pacific)
# # Extract residuals from model
# pcz2.pce3.p.partial <- partialResid(rPC2.zos.pac ~ rPC3.env.global.pac, pcz2.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# Extract residuals from model
pcz2.fc2.p.partial <- partialResid(rPC2.zos.pac ~ rFC2.global.pac, pcz2.test.p.1, site_means_Pacific) 
# Add site to residuals data frame 
Site <- site_means_Pacific$Site
...
# site_means_Pacific$rPCz1.rFC1 <- site_means_Pacific$rPC1.zos.pac * site_means_Pacific$rFC1.global.pac
# peri.test.p.11.x <- lm(rperiphyton.perg.pac ~ 
#                        + rPC1.env.global.pac + rPC2.env.global.pac + rPC3.env.global.pac + rFC1.global.pac + rFC2.global.pac
#                      + rPC1.zos.pac + rPC2.zos.pac
#                      + rPCz1.rFC1
#                      , data = site_means_Pacific)# Extract residuals from model
# peri.pcz1.p.partial <- partialResid(rperiphyton.perg.pac ~ rPC1.zos.pac, peri.test.p.11.x, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
peri.area.test.p.1 <- lm(rperiphyton.area.pac ~ 
                         ...
                         + rPC1.zos.pac + rPC2.zos.pac
                         , data = site_means_Pacific)
peri.area.pcz1.p.partial <- partialResid(rperiphyton.area.pac ~ rPC1.zos.pac, peri.area.test.p.1, site_means_Pacific) 
# Add site to residuals data frame 
Site <- site_means_Pacific$Site
...
peri.area.pcz2.p.partial <- partialResid(rperiphyton.area.pac ~ rPC2.zos.pac, peri.area.test.p.1, site_means_Pacific) 
# Add site to residuals data frame 
Site <- site_means_Pacific$Site
...
#                          + rPC1.zos.pac + rPC2.zos.pac
#                          , data = site_means_Pacific)
# peri.area.pce2.p.partial <- partialResid(rperiphyton.area.pac ~ rPC2.env.global.pac, peri.area.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# # First fit model at site level
# peri.perg.pcz2.p.partial <- partialResid(rperiphyton.perg.pac ~ rPC2.zos.pac, peri.test.p.11.x, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# peri.pce1.partial <- partialResid(rperiphyton.perg.pac ~ rPC1.env.global.pac, peri.test.p.11.x, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# peri.fca2.p.partial <- partialResid(rperiphyton.perg.pac ~ rFC2.global.pac, peri.test.p.11.x, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
#                     , data = site_means_Pacific)
# # Extract residuals from model
# meso.pce3.p.partial <- partialResid(rmesograzer.mass.perg.pac ~ rPC3.env.global.pac, meso.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# # Extract residuals from model
# meso.fc2.p.partial <- partialResid(rmesograzer.mass.perg.pac ~ rFC2.global.pac, meso.test.p.1, site_means_Pacific) 
# # Add Ocean and site to residuals data frame 
# Ocean <- site_means_Pacific$Ocean
# Site <- site_means_Pacific$Site
...
# # Extract residuals from model
# meso.pcz1.p.partial <- partialResid(rmesograzer.mass.perg.pac ~ rPC1.zos.pac, meso.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
# # Extract residuals from model
# meso.perg.pcz2.p.partial <- partialResid(rmesograzer.mass.perg.pac ~ rPC2.zos.pac, meso.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
...
meso.area.test.p.1 <- lm(rmesograzer.mass.area.pac ~ 
                         ...
                         , data = site_means_Pacific)
# Extract residuals from model
meso.pcz1.p.partial <- partialResid(rmesograzer.mass.area.pac ~ rPC1.zos.pac, meso.area.test.p.1, site_means_Pacific) 
# Add site to residuals data frame 
Site <- site_means_Pacific$Site
meso.pcz1.p.partial <- cbind(meso.pcz1.p.partial, Site)
meso.pcz1.p.partial.plot <- ggplot(meso.pcz1.p.partial, aes(x = xresid, y = yresid, col = "blue")) +
...
# Extract residuals from model
meso.area.pcz2.p.partial <- partialResid(rmesograzer.mass.area.pac ~ rPC2.zos.pac, meso.area.test.p.1, site_means_Pacific) 
# Add site to residuals data frame 
Site <- site_means_Pacific$Site
...
#                          , data = site_means_Pacific)
# # Extract residuals from model
# meso.area.pce3.p.partial <- partialResid(rmesograzer.mass.area.pac ~ rPC3.env.global.pac, meso.area.test.p.1, site_means_Pacific) 
# # Add site to residuals data frame 
# Site <- site_means_Pacific$Site
